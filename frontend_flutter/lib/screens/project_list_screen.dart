...
class _ProjectListScreenState extends State<ProjectListScreen> {
  ...
  Set<String> _evaluatedProjectIds =
      {}; // Track which projects the current evaluator has already graded
  ...
  Future<void> _fetchProjects({bool showLoader = true}) async {
    ...
    try {
      ...
      if (role == 'student') {
        ...
        final teacherId = (role == 'evaluator' || role == 'profesor')
            ? userId
            : null;
        final results = await Future.wait<dynamic>([
          ...
          _apiService.getAssignments(
            teacherId:
                ((role == 'teacher' || role == 'profesor') ||
                    (role == 'evaluator' || role == 'profesor'))
                ? userId
                : null,
            evaluatorId: (role == 'evaluator' || role == 'profesor')
                ? userId
                : null,
          ), // El backend filtra y devuelve la lista exacta.
          if ((role == 'evaluator' || role == 'profesor'))
            _apiService.getEvaluationsByEvaluator(userId!)
          else
            Future.value(<Evaluation>[]),
          _apiService
              .getClassrooms(), // Fetch all classrooms for grouping names
        ]);
      ...
      if (mounted && widget.userId != null && widget.userId!.isNotEmpty) {
        try {
          final List<AppNotification> notifs = await _apiService
      ...
      child: Scaffold(
            ...
            if ((widget.role.toLowerCase() == 'teacher' ||
              ...
              IconButton(
                ...
                onPressed: () => Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (_) =>
                        TeacherDashboardScreen(teacherId: widget.userId ?? ''),
                  ),
                ),
              ),
          ],
        ...
        body: RefreshIndicator(
          ...
          child: _isLoading
              ...
              : hasDualTabs
              ? TabBarView(
                  children: [_buildProjectTab(), _buildAssignmentTab()],
                )
              : widget.role.toLowerCase() == 'student'
  ...
  Widget _buildStudentDashboard() {
    ...
    final assignmentsToShow = _allAssignmentsForStudent.where((a) {
      ...
      if (_studentStatusFilter == 'Pendientes')
          ...
          child: _isLoading && _pendingAssignments.isEmpty
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      Icon(
                        _studentStatusFilter == 'Entregados'
                      ...
                      Text(
                        'No hay tareas ${_studentStatusFilter.toLowerCase()}',
                      ),
                    ],
                  ),
                )
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final isSubmitted = _allProjects.any(
                                ...
                                Text(
                                  ...
                                  style: TextStyle(
                                    color: isExpired && !isSubmitted
                                        ? Colors.red
                                        : Colors.grey,
                                    fontWeight: isExpired && !isSubmitted
                                        ? FontWeight.bold
                                        : FontWeight.normal,
                                    fontSize: 12,
                                  ),
              ...
              if (widget.userId == null || widget.userId!.isEmpty) {
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                        'Debes iniciar sesión para unirte a una clase.',
                      ),
                    ),
                  );
                }
            ...
            children: [
              ...
              ElevatedButton(
                onPressed: () => Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (_) =>
                        AssignmentCreationScreen(teacherId: widget.userId!),
                  ),
                ).then((_) => _fetchProjects()),
            ...
            Expanded(
              child: FutureBuilder<List<dynamic>>(
                future: Future.wait([
                  ...
                  if (a.classroomId != null)
                    _apiService.getClassMembers(a.classroomId!)
                  else
                    Future.value(<ClassEnrollment>[]),
                ]),
                builder: (context, snapshot) {
                  ...
                  final enrollments =
                      (snapshot.data?[1] as List<ClassEnrollment>?) ?? [];

                  if (projects.isEmpty && enrollments.isEmpty)
                    return const Center(
                      child: Text(
                  ...
                  final submittedStudentIds = projects
                      .map((p) => p.studentId)
                      .toSet();

                  // Enrolled students who haven't submitted
                  final pendingStudents = enrollments
                    ...
                    children: [
                      if (projects.isNotEmpty) ...[
                        ...
                        ...projects.map((p) {
                          ...
                          Color statusColor = status.toLowerCase() == 'evaluado'
                              ? Colors.blue
                              : Colors.orange;
                          ...
                          return ListTile(
                            ...
                            subtitle: Text(
                              'Estado: $status - Categoría: ${p.category ?? "Proyecto"}',
                            ),
                            trailing: const Icon(
  ...
  Future<void> _exportToCsv(Assignment a) async {
    try {
      AppTheme.showCustomSnackBar(
        context,
        'Generando reporte CSV, por favor espera...',
      );
      ...
      buffer.write('\uFEFF');
      buffer.writeln(
        'ID Proyecto,Equipo/Alumno,Categoria,Status,Puntaje General,Retroalimentacion',
      );
      ...
      for (var p in projects) {
        ...
        if (p.status?.toLowerCase() == 'evaluado') {
          ...
          if (eval != null) {
            ...
            feedbackStr =
                eval.feedback?.replaceAll(',', ';').replaceAll('\n', ' ') ??
                'N/A';
          }
        }
        ...
        buffer.writeln(
          '$idStr,$safeTeam,$safeCat,${p.status ?? "Pendiente"},$scoreStr,$feedbackStr',
        );
      }
      ...
      if (mounted)
        AppTheme.showCustomSnackBar(
          context,
          'Reporte generado. Abriendo archivo...',
        );
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          const Text(
            'Cuando se suban proyectos aparecerán en esta lista',
            style: TextStyle(color: Colors.grey),
          ),
          const SizedBox(height: 24),
          ElevatedButton.icon(
          ...
          if (widget.role.toLowerCase() == 'student') ...[
            ...
            OutlinedButton.icon(
              onPressed: () => Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (_) => StudentUploadScreen(
                    studentId: widget.userId ?? 'student_1',
                  ),
                ),
              ),
  ...
  Widget _buildIcon(String category) {
    ...
    if (category.contains('Hardware') || category.contains('IoT'))
  ...
  Widget _buildStatusBadge(bool isEvaluated) {
    return Container(
      ...
      child: Text(
        ...
        style: TextStyle(
          fontSize: 10,
          color: isEvaluated ? Colors.green[800] : Colors.orange[800],
          fontWeight: FontWeight.bold,
        ),
      ),
    );
          ...
          if ((widget.role.toLowerCase() == 'evaluator' ||
            ...
            ListTile(
              ...
              onTap: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => RubricManagementScreen(
                    teacherId: widget.userId ?? 'teacher_1',
                  ),
                ),
              ),
            ),
          ...
          if ((widget.role.toLowerCase() == 'teacher' ||
            ...
            ListTile(
              ...
              onTap: () async {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Generando reporte PDF global...'),
                  ),
                );
