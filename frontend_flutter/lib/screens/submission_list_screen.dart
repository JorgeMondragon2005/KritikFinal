...
class _SubmissionListScreenState extends State<SubmissionListScreen> {
  ...
  Future<void> _loadSubmissions() async {
    ...
    try {
      ...
      for (var p in projects) {
        if (p.id != null) {
          evaluations[p.id!] = await _apiService.getEvaluationByProjectId(
            p.id!,
          );
        }
      }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error al cargar entregas: $e')));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Entregas: ${widget.assignmentTitle}')),
      body: _isLoading
          ...
          ? Center(
              child: Column(
                ...
                children: [
                  ...
                  const Text(
                    'No hay entregas para esta convocatoria todavía',
